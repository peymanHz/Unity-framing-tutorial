...
#pragma warning restore 414
        ...
        if (itemsDetails != null)
        {
            switch (itemsDetails.itemType)
            {
                case ItemType.Seed:
                    ...
                    {
                        ProcessPlayerClickInputSeed(itemsDetails, gridPropertyDetails);
                    }
                    break;
    ...
    private void ProcessPlayerClickInputSeed(ItemsDetails itemsDetails, GridPropertyDetails gridPropertyDetails)
    {
        if (itemsDetails.canBeDropped && gridCursor.CursorPositionIsValid && gridPropertyDetails.daysSinceDug > -1 && gridPropertyDetails.seedItemCode == -1)
        {
            PlantSeedAtCursor(gridPropertyDetails, itemsDetails);
        }
    ...
    private void PlantSeedAtCursor(GridPropertyDetails gridPropertyDetails, ItemsDetails itemsDetails)
    {
        //process if we have cropdetails for the seed
        if (GridPropertyManager.Instance.GetCropDetails(itemsDetails.itemCode) != null)
        {
            //update grid properties with seed details
            gridPropertyDetails.seedItemCode = itemsDetails.itemCode;
    ...
    private void ProcessPlayerClickInputTool(GridPropertyDetails gridPropertyDetails, ItemsDetails itemsDetails, Vector3Int playerDirection)
    {
        ...
        switch (itemsDetails.itemType)
        {
            ...
            case ItemType.Collecting_Tool:
                if (gridCursor.CursorPositionIsValid)
                {
                    CollectInPlayerDirection(gridPropertyDetails, itemsDetails, playerDirection);
                }
                break;
    ...
    private void CollectInPlayerDirection(GridPropertyDetails gridPropertyDetails, ItemsDetails equippedItemsDetails, Vector3Int playerDirection)
    {
        //trigger animation
        StartCoroutine(CollectInPlayerDirectionCoroutine(gridPropertyDetails, playerDirection, equippedItemsDetails));
    }

    private IEnumerator CollectInPlayerDirectionCoroutine(GridPropertyDetails gridPropertyDetails, Vector3Int playerDirection, ItemsDetails equippedItemsDetails)
    {
        PlayerInputDisabled = true;
        playerToolUseDisabled = true;

        ProcessCropWithEquippedItemInPlayerDirection(gridPropertyDetails, equippedItemsDetails, playerDirection);
    ...
    /// <summary>
    /// methoed processes crop with equipped item in player direction
    /// </summary>
    private void ProcessCropWithEquippedItemInPlayerDirection(GridPropertyDetails gridPropertyDetails, ItemsDetails equippedItemsDetails, Vector3Int playerDirection)
    {
        ...
        //excute process tool action for crop
        if (crop != null)
        {
            switch (equippedItemsDetails.itemType)
            {
                case ItemType.Collecting_Tool:
                    crop.ProcessToolAction(equippedItemsDetails, isPickingRight, isPickingLeft, isPickingDown, isPickingUp);
                    break;
            }
        }
    }
